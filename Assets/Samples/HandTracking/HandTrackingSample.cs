...
public class HandTrackingSample : MonoBehaviour
{
    ...
    void DrawJoints(Vector3[] joints)
    {
        ...
        for (int i = 0; i < HandLandmarkDetect.JOINT_COUNT; i++)
        {
            draw.Cube(worldJoints[i], lineSize * 2);
        }
	...
	void RequiredJoints()
	{
		...
		requiredJointValues.zeroPointPosition = worldJoints[connections[0]];
		requiredJointValues.sevenPointPosition = worldJoints[connections[7]];
		requiredJointValues.ninePointPosition = worldJoints[connections[9]];
		requiredJointValues.fifteenPointPosition = worldJoints[connections[15]];
		requiredJointValues.twentythreePointPosition = worldJoints[connections[23]];
		requiredJointValues.thirtyOnePointPosition = worldJoints[connections[31]];
		requiredJointValues.thirtyFivePointPosition = worldJoints[connections[35]];
		requiredJointValues.fortyOnePointPosition = worldJoints[connections[41]];
		
		// requiredJointValues.zeroPointPosition = worldJoints[connections[0]];
		// requiredJointValues.fourPointPosition = worldJoints[connections[4]];
		// requiredJointValues.fivePointPosition = worldJoints[connections[5]];
		// requiredJointValues.eightPointPosition = worldJoints[connections[8]];
		// requiredJointValues.ninePointPosition = worldJoints[connections[9]];
		// requiredJointValues.twelvePointPosition = worldJoints[connections[12]];
		// requiredJointValues.sixteenPointPosition = worldJoints[connections[16]];
		// requiredJointValues.seventeenPointPosition = worldJoints[connections[17]];
		// requiredJointValues.twentyPointPosition = worldJoints[connections[20]];
